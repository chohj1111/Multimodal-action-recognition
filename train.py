...
file_handler = logging.FileHandler(
    "logs/train_logs_" + str(datetime.datetime.now())[:-7] + ".log"
)
stdout_handler.setLevel(logging.INFO)
stdout_handler.setFormatter(formatter)
...
def runner(epochs, model, train_iterator, valid_iterator, optim, writer, config):
    ...
    for epoch in range(epochs):
        ...
        add_to_writer(
            writer, epoch, train_loss, valid_loss, train_stats, valid_stats, config
        )
...
def main():
    ...
    config["log_path"] = os.path.join(
        config["log_path"], str(datetime.datetime.now())[:-7]
    )
    ...
    train_dataset = ds_fctr.get_ds_factory()(**factory_args)

    factory_args = {"subset": "test"}
    valid_dataset = ds_fctr.get_ds_factory()(**factory_args)

    logger.info("Loading data and dataloaders")
    _, _, vocab = get_Dataset_and_vocabs(
        config["data"]["path"],
    ...
    collate_fn_ = partial(
        collate_fn,
        device=device,
        text_pad_value=vocab["text_vocab"]["<pad>"],
        audio_pad_value=0,
        audio_split_samples=config["audio_split_samples"],
    )
